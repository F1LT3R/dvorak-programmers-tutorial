...
const transformLeads = (leadLines, type) =>
  new Promise((resolve, reject) => {
    try {
...
function beginLesson(lines, callback) {
  ...
  function onKeyPress(ch, key) {
    // const atEdge = col > 0 && col % cols >= cols - 1;

    const col = cursorColumn;
    const cols = process.stdout.columns - 1;
    const modCol = col % cols;
    const atEdge = col > 0 && modCol === 0;
    const notEdge = modCol < cols;
    // beforeEdge && console.log('be')
    ...
    if (key && key.hasOwnProperty("name")) {
      ...
      if (key.name === "backspace") {
        let changed;
        const inBounds = col > 0 && modCol < cols;
    ...
    if (ch === currentLine[cursorColumn]) {
      if (!returned) {
        ...
        notEdge && process.stdout.write(clc.move.left(1));
        process.stdout.write(clc.erase.lineRight);
        process.stdout.write(pass(ch).replace(/\s/, dim("⠐")));
        notEdge && process.stdout.write(dim("_"));
        trace += "1";
      }
    } else {
      if (!returned) {
...
(fileToLoad && startWithFile(fileToLoad)) || startWithMenu();
